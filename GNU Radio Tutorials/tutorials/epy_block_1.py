...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def work(self, input_items, output_items):
        ...
        for index in range(len(output_items[0])):
            ...
            #make sure the list is not-empty, and if the current input sample is greater than or equal to the next
            if(len(relativeOffsetList) > 0 and index >= relativeOffsetList[0]):
                ...
                #a detect tag has not been seen, so continue to increase the output counter
